...
{
    public class VSLeakDetector : ICloneable
    {
        #region Members
        private SerialPort mySerialPort;
        //public SerialPort mySerialPort { get; set; } // <-- this one. ic
        ...
        public bool WaitForFineTest(ref string retVal)
        {
            ...
            Repeat: retVal = Query(command, ref Success);
                if (Success)
                {
                    ...
                    counter++;
                    if (retVal.Trim().ToLower() != "midstage" && counter <= 120)
                    {
                        Thread.Sleep(1000);
                        goto Repeat;
                    }
        ...
        public bool WaitForStdLeakState(ref string retVal)
        {
            try
            {
                ...
                int counter = 0;
            Repeat:
                //retVal = Query("?VALVESTATE", ref Success);
                retVal = Query(command, ref Success);
                if (Success)
                {
                    Trace.WriteLine(retVal);
                    counter++;
                    //if ((retVal!= "STDLEAK") && (counter <= 60))
                    if ((!retVal.Contains("STDLEAK")) && (counter <= 60))
                    {
                        Thread.Sleep(1000);
                        retVal = null;
                        goto Repeat;
                    ...
                    throw new Exception("Error when sending command ");
            }
            catch (Exception)
            {
        ...
        public string ReadLeakRate()
        {
            try
            {
                ...
                string retVal = Query("?LEAK", ref Success);
                if (Success)
                {
                    return Convert.ToString(retVal);
                }
                ...
                {
                    throw new Exception("Error when sending command ?LEAK"); 
                }
        ...
        public bool WaitForStabilizedReading(ref string retVal)
        {
            try
            {
                return this.WaitForStabilizedReading(ref retVal, 200, 0.95, 10);
            }
            catch (Exception)
            {
        ...
        public void WaitForAtmosphericPressure(int minPressure, int timeout)
        {
            ...
            //    while (tpPressure < minPressure)
            //    {
            //        tpPressure = GetTestPortPressure();

            //         if (count > timeout)
            //        {
            //            throw new Exception("fail pressure");
            //            trigger = false;
            //        }
